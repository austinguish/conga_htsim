...
public:
    ...
    struct CongaFeedback {
        uint32_t leafId;
        uint32_t coreId;
        double congestionMetric;
        bool hasValidFeedback;

        CongaFeedback() : leafId(0), coreId(0), congestionMetric(0.0), hasValidFeedback(false) {
        }
    } congaFeedback;

    // Helper methods for feedback
    void setCongaFeedback(uint32_t leaf, uint32_t core, double metric) {
        congaFeedback.leafId = leaf;
    ...
    bool hasCongaFeedback() const {
        return congaFeedback.hasValidFeedback;
    }
